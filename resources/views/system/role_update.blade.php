...
<body class="mainbody">
    <div class="layui-container-tab">
        <div class="layui-row topfloatbar">
            <button type="button" class="layui-btn layui-btn-primary layui-btn-sm topfloatbar-title" disabled>标题：角色数据修改</button>
        ...
        <form class="layui-form layui-form-pane" action="">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input type="text" name="rolename" id="rolename" lay-verify="required" maxlength="64" autocomplete="off" placeholder="角色的标识名称" class="layui-input" value="{{ $resarr['data']->rolename ?? '' }}">
                </div>
            </div>
                ...
                <div class="layui-input-block">
                    ...
                    <ul class="layui-input-block" style="margin-top: 10px ;">
                        <li>
                            ...
                            <input type="checkbox" class="parent" name="selectmenu[]" value="{{$resarr['menulist'][$keys]['m']['id']??''}}" title="{{$resarr['menulist'][$keys]['m']['menuname']??''}}" lay-skin="primary" {{$resarr['menulist'][$keys]['m']['check']??''}}>
                            @endif
                            @if (isset($resarr['menulist'][$keys]['c']))
                            <ul>
                                @foreach ($resarr['menulist'][$keys]['c'] as $keyss => $valss)
                                <input type="checkbox" class="child" name="selectmenu[]" value="{{$valss['id']??''}}" title="|------- {{$valss['menuname']??''}}" lay-skin="primary" {{$valss['check']??''}}><br>
                                @endforeach
                            </ul>
                            @endif
                        </li>
            ...
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <table class="layui-table" lay-size="sm">
                        ...
                        @else
                        <tr>
                            <td><input type="checkbox" name="selectnode[]" value="{{$valn->id??''}}" lay-skin="primary" {{$valn->check??''}}></td>
                            <td>{{$valn->nodename??''}}</td>
