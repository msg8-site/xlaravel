...
define('COMM_SECIP', env('COMM_SECIP', true));  //是否为双层ip，默认 false 仅判断REMOTE_ADDR
define('COMM_DBLOCK', env('COMM_DBLOCK', false));  //数据库迁移回滚锁，防止误删除， true 允许回滚删除数据表，其他值如 false ，不允许回滚删除
define('COMM_SYSNAME', env('COMM_SYSNAME', 'xlaravel后台框架'));  //平台名称
define('COMM_CODETYPE', env('COMM_CODETYPE', 'imgcode'));  //验证类型， google 为谷歌动态码， imgcode 为图形验证码
